...
このプロジェクトは、[anime-song/ChordAI](https://github.com/anime-song/ChordAI) で公開されている学習済みモデルを使用させていただいており、android-cat によって開発・メンテナンスされています。
...
- 波形表示とコードタイムラインの同期表示
- 再生 / 逆再生 / 停止 / シーク操作
- 577種類のコードタイプ + ベース音の推定（N.C. 含む）
- キー推定（25キー）と推定キーに基づくアクシデンタル（♯/♭）自動補正
- タイムライン上のキー表示レーン
...
初回起動時にモデルが自動ダウンロードされますが、手動でダウンロードすることもできます。

[ここ](https://huggingface.co/anime-song/ChordAI/resolve/main/chordestimation.tar.gz?download=true)からダウンロードして解凍後、`model/` フォルダーに配置してください。
...
2. 初回起動時は仮想環境の作成、依存ライブラリのインストール、モデルのダウンロードが自動的に行われます。
3. アプリケーションが起動します。
...
# アプリケーションを起動（モデルが未ダウンロードの場合は自動取得されます）
python main.py
```

1. アプリが起動したら、音声ファイルを開きます
2. 自動的にコード解析が実行されます
3. タイムライン上にコードとキーが表示され、再生しながら確認できます
...
├── audio_processor.py     # 音声読み込み・CQT前処理・コード/キー変換
├── chord_model.py         # モデルの読み込み・ダウンロード・推論
├── player.py              # sounddevice による音声再生エンジン
├── index.json             # コードインデックス（ID → コード名, 577種）
├── requirements.txt       # Python 依存パッケージ
├── model/
│   └── chordestimation/   # 学習済みコード推定モデル (SavedModel)
└── ui/
    ├── __init__.py
    ├── main_window.py     # メインウィンドウ
    ├── player_controls.py # 再生コントロール UI
    ├── styles.py          # スタイルシート
    ├── timeline_widget.py # コード/キータイムライン表示
